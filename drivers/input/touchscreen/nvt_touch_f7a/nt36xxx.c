...
#if WAKEUP_GESTURE
       		ret = request_threaded_irq(client->irq, NULL, nvt_ts_irq_handler, ts->int_trigger_type | IRQF_ONESHOT | IRQF_PERF_CRITICAL |IRQF_NO_SUSPEND, client->name, ts);
#else
     		ret = request_threaded_irq(client->irq, NULL, nvt_ts_irq_handler, ts->int_trigger_type | IRQF_PERF_CRITICAL | IRQF_ONESHOT, client->name, ts);
#endif
		if (ret != 0) {
